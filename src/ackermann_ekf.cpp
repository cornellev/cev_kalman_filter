...
public:
    OdomSensor(V state, M covariance, std::vector<std::shared_ptr<Model>> dependents)
...
public:
    AckermannEkfNode(): Node("AckermannEkfNode") {
        ...
        odom_sub = this->create_subscription<cev_msgs::msg::SensorCollect>("sensor_collect", 1,
            std::bind(&OdomSensor::msg_handler, &odom, _1));
...
private:
    ...
    OdomSensor odom = OdomSensor(V::Zero(), M::Identity() * .1, {model});
};
